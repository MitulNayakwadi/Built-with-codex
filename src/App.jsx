      ...
      <main className="p-4 space-y-4">
        {activeTab === 'feed' && (
          <FeedTab
            ...
            onComment={(postId, text) => {
              dispatch({ type: 'ADD_COMMENT', postId, comment: { id: crypto.randomUUID(), userId: me.id, text, createdAt: new Date().toISOString() } });
        ...
        {activeTab === 'profile' && (
          <ProfileTab
            ...
            onChat={(text) => dispatch({ type: 'ADD_CHAT', message: { id: crypto.randomUUID(), userId: me.id, text, ts: new Date().toISOString() } })}
...
function SplashAndOnboarding({ loading, mode, onLogin, onCreate, onFinish }) {
  ...
  const finish = () => {
    const group = form.groupMode === 'join'
      ...
      : { id: crypto.randomUUID(), name: `${form.groupName} ${form.groupEmoji}`, members: 1, activity: 'New' };
    onFinish({ group });
  };
...
function PostCard({ post, users, onReaction, onComment, onProfile, onShare }) {
  ...
  const reactions = [
    ...
    { id: 'clap', icon: Hands, label: 'Clap', color: 'hover:bg-cyan-500/20' },
...
function BottomNav({ active, onChange }) {
  const tabs = [
    ...
    { id: 'progress', icon: ChartColumn, label: 'Progress' },
